...
export default function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-50 bg-cream border-b-2 border-gold transition-shadow duration-300 ${scrolled ? 'shadow-md' : ''}`}>
      <nav className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <ul className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map(({ label, href }) => (
            <li key={href}>
              <a href={href} className="text-navy font-medium hover:text-orange transition-colors text-sm">{label}</a>
            </li>
          ))}
        </ul>
